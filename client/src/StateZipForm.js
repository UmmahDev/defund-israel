...
We are currently spending BILLIONS of dollars EVERY YEAR on Israel. I do not want my money going towards the ethnic cleansing and genocide of Palestinians. Not a dollar more.`; // Replace this with your actual email body content
...
const StateZipForm = () => {
    ...
    const emailSubject = 'Constituent Demand - DEFUND ISRAEL';
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await fetch(`https://yra6hejx6k.execute-api.us-east-1.amazonaws.com/dev/get-representatives?zipcode=${zipCode}`);
            if (response.ok) {
                ...
                const mailtoLink = generateMailtoLink(allEmails, emailBody(state, zipCode));

                setEmailInfo({ emails: allEmails, body: emailBody(state, zipCode) });
    ...
    const generateMailtoLink = (emails, body) => {
        return `mailto:${emails.join(',')}?subject=Constituent Demand - DEFUND ISRAEL&body=${encodeURIComponent(body)}`;
    };
